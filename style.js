function drop() {
    let dropdownmenu = document.getElementById("drop");
    if (dropdownmenu.className === "topnav") {
        dropdownmenu.className += " responsive";
    } else {
        dropdownmenu.className = "topnav";
    }
}


// const allimg = document.getElementsByTagName('img');

// for (let img of allimg) {
//     img.src = 'https://images.unsplash.com/photo-1630905119003-329447458f85?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTJ8fGJvdXRpcXVlfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=800&q=60'
// }

// const stext = document.getElementsByTagName('p');
// for (let p of stext) {
//     p.innerText = 'Hope De';
// }

// const hhh = document.getElementsByTagName('h2');
// for (let h2 of hhh) {
//     h2.innerText = 'kkk'
// }

// const kkk = document.getElementsByClassName('work');
// for (let work of kkk) {
//     work.innerText = 'hnahhd';
// }


// qurey sellector //

// const bbb = document.querySelector('p')
// bbb.innerText = 'God bless you';

// const links = document.querySelectorAll('a');
// for (let link of links) {
//     link.innerText = 'click'
// }

// const bb = document.querySelector('.last p');
// bb.innerText = 'hhh'

// const hhh = document.querySelector('h1');
// hhh.innerHTML = '<em>i love you</em>';

// const new = document.querySelector('#drop');
// new.id = 'cool';

// const neww = document.querySelector('a');
// console.log(neww.href)
// console.log(neww.getAttribute('href'));

// const neww = document.querySelector('a');
// console.log(neww.setAttribute('href', 'http://www.google.com'));

// const nee = document.querySelector('input[type="text"]');
// nee.type = 'select'

// const neww = document.querySelector('h1');
// neww.style.color = 'yellow'

// for single //
// const para = document.querySelector('p');
// para.style.color = 'pink';
// para.style.fontSize = '50px';
// para.style.fontWeight = '900';

// for all //

// const para = document.querySelectorAll('p');
// for (let p of para) {
//     p.style.color = 'green';
// }

// const cont = document.querySelector(#container);
// cont.style.textAlign = 'center';

// const image = document.querySelector('img')
// image.style.width = '150px'
// image.style.borderRadius = '50%'

// const colorss = document.querySelectorAll('span');
// for(let i = 0; i < colorss.length; i++) {
//     colorss[i].style.color = colors[i]
// }

// to add from css //
// const ccc = document.querySelector('h2');
// ccc.setAttribute('class', 'black');
// const cctt = ccc.getAttribute('class')
// ccc.setAttribute('class', `${cctt} border`)


// // to add from css easy //

// const ccc = document.querySelector('h2');
// ccc.classList.add('black', 'border')

// // to remove //
// ccc.classList.remove('border')

// // to check if element is in //
// ccc.classList.contains('border');

// to check toggle means (to check) //
// ccc.classList.toggle('black');


// const newclass = document.querySelectorAll('li')
// for (let li of newclass) {
//     li.classList.add('black')
// }

// const newlist = document.querySelectorAll('li');
// for(let li of newlist) {
//     li.classList.toggle('highlight')
// }

//  for parent elements //

// const bold = document.querySelector('p');
// bold.parentElement;

// const image = document.createElement('img');
// image.src = 'data:image/jpeg;base64,'
// document.body.appendChild(image);
// image.classList.add('imagode');

// to create new element from java to your html //

// const newh3 = document.createElement('h3')
// newh3.innerText = 'you killed it';
// document.body.appendChild(newh3)
// newh3.style.color = 'yellow'

// const newh1 = document.createElement('h1')
// document.body.appendChild(newh1)
// newh1.innerText = 'are you okay';
// newh1.classList.add('black', 'border')

// const nwe1 = document.createElement('h1');
// document.body.appendChild(nwe1)
// nwe1.innerText = 'hello'
// nwe1.classList.add('black', 'border')

// to append directly //

// const new1 = document.querySelector('h2');
// new1.append(' i am the second h1');
// new1.classList.add('black')

// const new1 = document.querySelector('p');
// new1.append(' add me to paragraph')
// new1.classList.add('black', 'border')

// const new2 = document.createElement('h1')
// new2.append('add hi')
// new2.prepend('p')

// to add element after another element //

// const newh2 = document.createElement('h2');
// newh2.append('are the best in the world');
// newh2.classList.add('black')
// const new1 = document.querySelector('h1');
// new1.insertAdjacentElement('afterend', newh2);

// // easy way to add //
// const nwe3 = document.createElement('h2')
// nwe3.append('i am new h3')
// new1.before(nwe3)
// new1.after(nwe3)


// const btn = document.querySelector('#container')
// for(let i = 0; i <= 100; i++) {
//     const newbtn = document.createElement('button')
//     newbtn.innerText = 'yes'
//     btn.appendChild(newbtn)
// }


// const nnn = document.querySelector('new3')
// body.removeChild(new3);

// const hhhh = document.querySelector('button');
// hhhh.remove();

// const new4 = document.querySelector('h1');
// new4.remove();































